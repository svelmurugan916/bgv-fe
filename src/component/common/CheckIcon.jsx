...
const CheckIcon = ({ status, label }) => {
    ...
    return (
        <div className="group/icon relative cursor-help">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center border transition-all duration-300 ${colors[status] || 'bg-slate-50 text-slate-300 border-slate-100'}`}>
                {getTaskIcon(label)}
                ...
                {status === 'unassigned' && (
                    <>
                        ...
                        <div className="absolute -bottom-0.5 -left-0.5">
                            <AlertCircle size={10} className="text-red-500 fill-white" />
                        </div>
                    </>
                )}
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-slate-800 text-white text-[10px] font-bold rounded opacity-0 group-hover/icon:opacity-100 transition-all scale-90 group-hover/icon:scale-100 whitespace-nowrap pointer-events-none z-30 shadow-xl">
                <div className="flex flex-col items-center">
                    <span>{label?.toUpperCase()}: <span className="uppercase ml-1 opacity-70">{status?.replace('unabletoverify', 'Unable to Verify').replace('onhold', 'On Hold').replace('waitingforcandidate', 'Waiting for Candidate').replace('needsreview', 'Needs Review')}</span></span>
                    {status === 'unassigned' && (
